...
def Navier_Stokes_FDM_cavity_internal_non_dim(U, lid_velocity, nu, L):
    ...
    # Bernoulli equation.
        ...
        # -assume incompressible and calculate pressure field from u/v
...
def model_routine_cavity(args, model, training_dataset, testing_dataset=None):
    ...
    #dlw = dynamic_loss_weighting(LR=args['base_lr'],alpha=0.16)
    ...
    for epoch in range(epochs): 
        ...
        for batch_n, batch in enumerate(training_dataloader):
            ...
            # pde_loss1 = dlw.params[0]*loss_func(g, Du_dx, torch.zeros_like(Du_dx))[0]
            # pde_loss2 = dlw.params[1]*loss_func(g, Dv_dy, torch.zeros_like(Dv_dy))[0]
            # pde_loss3 = dlw.params[2]*loss_func(g, continuity_eq, torch.zeros_like(continuity_eq))[0]
            ...
            if args['loss_weighting'] == 'scaled' and batch_n == 0 and epoch == 0:
                    params = torch.tensor(pde_l1_unweighted + pde_l2_unweighted + pde_l3_unweighted)/(3*torch.tensor([pde_l1_unweighted, pde_l2_unweighted, pde_l3_unweighted]))
                    
            pde_loss1 = params[0]*pde_l1_unweighted
            pde_loss2 = params[1]*pde_l2_unweighted
            pde_loss3 = params[2]*pde_l3_unweighted

            total_avg_pde_loss = (pde_loss1 + pde_loss2 + pde_loss3)/3
            total_weighted_loss = xy_weight*loss_total + pde_weight*total_avg_pde_loss
            
            if batch_n == 0 & epoch == 0:
                if  args['loss_weighting'] == 'dynamic':
                    l01 = pde_loss1.data  
                    l02 = pde_loss2.data
                    l03 = pde_loss3.data
            ...
            #dlw.calculate_params(model.parameters(),pde_loss1,pde_loss2,pde_loss3)
            if  args['loss_weighting'] == 'dynamic':
            ...
            print(f'Epoch: {epoch}, Batch: {batch_n + 1}, Last LP Training Loss: {loss_total.item()}, Estimated Time Left {(batch_end_time - batch_start_time)*(epochs-epoch+1)*(len(training_dataloader)-batch_n+1)/3600:.2f}hrs with PDE Loss Weights {params[0].item():.3f}, {params[1].item():.3f}, {params[2].item():.3f}'
                  , end=print_end)
...
if __name__ == '__main__':
    ...
    dataset = Cavity_2D_dataset_handling_v2(dataset_path, name='cavity', train=True, sub_x = 4,
                                        normalize_y=dataset_args['use-normalizer'], normalize_x = dataset_args['normalize_x'])
    eval_dataset = Cavity_2D_dataset_handling_v2(dataset_path, name='cavity_eval', train=False, sub_x = 4,
    ...
    training_args["save_dir"]               = 'gnot_cavity_v2'
    training_args["save_name"]              = 'ansatz_trial_v2'
    ...
    training_args['loss_weighting']         = 'dynamic' #'dynamic' #scaled #scaled, dynamic or none
